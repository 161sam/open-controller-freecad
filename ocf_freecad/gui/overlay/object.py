...
def _find_overlay_object(doc: Any) -> Any | None:
    if hasattr(doc, "getObject"):
        try:
            obj = doc.getObject(OVERLAY_OBJECT_NAME)
            if obj is not None:
                return obj
        except Exception:
            pass
    for obj in getattr(doc, "Objects", []):
        if getattr(obj, "Name", None) == OVERLAY_OBJECT_NAME or getattr(obj, "Label", None) == OVERLAY_OBJECT_LABEL:
            return obj
    return None
...
class ViewProviderOverlay:
    ...
    def ensure_attached(self, view_object: Any) -> None:
        ...
        self.root.setName("OCFOverlayRoot")
